...
| NSFW 自动判断 | `src/nsfw-auto.js` | 摘要进度为主、关键词抢当回合、事后缓冲；含入口小点的静默状态显示 |
| 世界书 NSFW 拦截 | `src/nsfw-worldbook.js` | 在 `WORLDINFO_ENTRIES_LOADED` 上就地剔除条目；只读不写，不改世界书 |
...
19 项结构测试 + 4 套 jsdom 面板测试通过。此次没有改变运行代码或视觉，也没有重复执行上一轮的浏览器流程；上一轮截图和宿主模拟验证见 [维护检查记录](review-2026-09-20.md)。后续功能修改仍需按影响范围运行宿主回归。

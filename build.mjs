...
const script = assemble('src/fruit-heart.template.js', { read, values: {
    ...
    __NSFW_AUTO_CSS__: JSON.stringify(read('src/nsfw-auto.css')),
    __NSFW_AUTO_CORE__: read('src/nsfw-auto.js'),
    __NSFW_AUTO_RUNTIME__: read('src/nsfw-auto-runtime.js'),
...
if (process.argv.includes('--local')) {
    ...
    fs.writeFileSync(path.join(dir, 'fruit-heart-local.json'), JSON.stringify({
        type: 'script', enabled: false, name: '果实之心丨NSFW关键词联动丨本地测试',
        id: 'b1772083-57d1-4718-93bd-7e5793a0ea10', content: script,
        info: '先停用原果实之心脚本，再导入并启用此本地测试版。设置中配置 NSFW 关键词联动。',

(async function () {
    ...
    const SCRIPT_VERSION = "20260918.3";
...
/flushvar final_message ||`;
    ...
    // 固定四种模型。tag 就是条目名里 # 后面那个，applyTag 会按它开关条目；
    // 【尾】不用特判：尾条目自己带 #GEMINI 3.8F / #GEMINI 3.1PRO / #DS，
    // appliedTag 精确匹配不上就关掉，所以选 Pro 自动关 Flash 尾，反之亦然。
    const MODELS = [
        { tag: 'GEMINI 3.8F', label: 'Gemini Flash' },
        { tag: 'GEMINI 3.1PRO', label: 'Gemini Pro' },
        { tag: 'CLAUDE', label: 'Claude' },
        { tag: 'DS', label: 'DeepSeek' },
    ];
    // 预设里存的 activeTag 可能是老的族名（GEMINI），不在四选一里。
    // 那就按族归到同族的第一个，别让下拉显示成空。
    function currentModel(config, models) {
        return models.find(item => item.tag === config.activeTag)
            || models.find(item => tagFamily(item.tag) === tagFamily(config.activeTag))
            || null;
    }
    function modelChoices(preset = activePreset()) {
        const have = new Set(allModelTags(preset));
    ...
    const root = jq(`<div id="${APP_ID}" role="dialog" aria-modal="true" aria-label="果实之心"><div class="fh-shell">
        <header class="fh-header"><span class="fh-brand-mark">${BALL_ICON}</span><div class="fh-brand"><div><strong>果实之心</strong><em>@KKM</em></div><small>${SUBTITLE}</small></div><div class="fh-header-meta"><button class="fh-theme" data-action="theme" aria-label="切换主题"></button><button class="fh-fullscreen" data-action="fullscreen" aria-label="面板全屏" aria-pressed="false">⛶</button><button class="fh-close" data-action="close" aria-label="关闭">×</button></div></header>
    ...
    function ecotEndTags() {
        const api = hostWindow.FruitHeartECoT || window.FruitHeartECoT;
        return api && typeof api.getEndTags === 'function' ? api.getEndTags() : ['</ECoT>', '</thinking>', '</think>'];
    }
    function wordRange() {
        const config = readConfig();
    ...
    const RELEASE_NOTES = [{"version":"20260918.3","date":"2026-09-18","changes":["新增连接模型与预设模型组联动，默认开启，可在设置关闭。","新增脚本更新页，可查看更新记录、检查并下载新版。","未知模型或不明确的分组保持原状；连接不变时保留手动选择。"]},{"version":"20260918.2","date":"2026-09-18","changes":["显示脚本版本；统一 Git 加载与本地测试副本。","修复分区同步重复提示、白天文字阴影和自定义字数。","移除底部关闭项，非全屏点击面板外关闭。"]}];
    ...
    jq(doc).on('keydown.fruitHeartKeys', event => {
    ...
    function showBall() {
        if (ballDock) return;
        ballDock = jq(`<div id="${APP_ID}-dock" class="${jumpNav ? 'has-jump' : ''}" style="--dock:${ballScale()}">`
        ...
        jq(hostWindow).off('resize.fruitHeartBall').on('resize.fruitHeartBall', () => {
            if (!ballDock) return;
    ...
    function dressBarEntry() {
        ...
        for (const node of barEntryButtons()) {
            ...
            if (!node.getAttribute('title')) node.setAttribute('title', BUTTON_NAME);
    ...
    function bindEntry() {
        ...
        sweepBarEntry();
        if (!entryBar) { jq(doc).off('click.fruitHeartEntry'); showBall(); return; }
        jq(doc).off('click.fruitHeartEntry').on('click.fruitHeartEntry', 'button', function (event) {
        ...
        const bound = (() => { if (!getEvent || !on) return false; const name = getEvent(BUTTON_NAME); if (!name) return false; on(name, open); return true; })();
        // 快捷回复栏是默认入口；认不出按钮、或用户自己选了浮球，就挂 🍎。
        // 认不出快捷回复栏按钮时，就算用户没勾浮球也得挂一个，否则面板彻底打不开
        if (entryBall || jumpNav || !entryBar || !bound) showBall();
    }
    let rollBound = false;
    function bindRoll() {
        if (rollBound) return;
    ...
    function destroy() {
        ...
        clearTimeout(searchTimer); clearInterval(syncTimer); jq(doc).off('click.fruitHeartEntry').off('keydown.fruitHeartKeys'); jq(hostWindow).off('resize.fruitHeartBall'); root.remove(); style.remove(); if (ballDock) ballDock.remove();
    }
    ...
    const entrySweepTimer = setInterval(() => { if (!doc.hidden) sweepBarEntry(); }, 2500);   // 关着要藏，开着要补图标，两种情况都得扫
})();

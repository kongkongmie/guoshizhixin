...
(function () {
  ...
  if (COMMON_JS) {
    ...
    const SCRIPT_VERSION = "20260919.11";
...
/flushvar final_message ||`;
    ...
    function renderSettings() {
        ...
        main.html(`${titleBlock('设置', '本脚本功能和结构借鉴了 @电波系 的日月西预设脚本、@NUE 的脚本。')}
          ...
          <section class="fh-card fh-mt"><div class="fh-card-head"><h3>外观与入口</h3></div>
            ...
            <label class="fh-toggle fh-mt"><span>🍎快捷栏入口</span><button class="fh-sw ${entryBar ? 'on' : ''}" data-entry-toggle="bar" role="switch" aria-checked="${entryBar}"></button></label>
    ...
    function ensureBarEntry() {
        ...
        button.tabIndex = 0;
        button.innerHTML = '<div class="qr--button-label">🍎快捷栏入口</div>';
    ...
    function dressBarEntry() {
        ...
        for (const node of barEntryButtons()) {
            ...
            if (!text) continue;
            text.nodeValue = text.nodeValue.replace(/🍎\s*(?:果实之心|快捷栏入口)?/, '🍎快捷栏入口');
        }
    }
    ...
    function bindEntry() {
        ...
        hideBall();
        sweepBarEntry();
        if (!entryBar) { jq(doc).off(`click${ENTRY_EVENT_NS}`); return; }
        ...
        // 悬浮猫咪只由自己的开关控制；快捷栏暂时识别不到时也不擅自显示。

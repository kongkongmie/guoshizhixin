...
(function () {
  ...
  if (COMMON_JS) {
    ...
    const SCRIPT_VERSION = "20260919.6";
...
/flushvar final_message ||`;
    ...
    function ensureBarEntry() {
        ...
        button.innerHTML = `<span class="fh-bar-cat">${BALL_ICON}</span><span>果实之心</span>`;
    ...
    function bindEntry() {
        ...
        jq(doc).off(`click${ENTRY_EVENT_NS}`).on(`click${ENTRY_EVENT_NS}`, 'button', function (event) {
    ...
    function destroy() {
        ...
        clearTimeout(searchTimer); clearInterval(syncTimer); jq(doc).off(`keydown${KEY_EVENT_NS}`); jq(hostWindow).off(`resize${BALL_EVENT_NS}`); root.remove(); style.remove();
    }

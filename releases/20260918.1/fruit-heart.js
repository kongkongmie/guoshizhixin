(async function () {
    ...
    const SUBTITLE = '适配果实预设 V6.0 及以上版本';
...
/flushvar final_message ||`;
    ...
    function modelChoices(preset = activePreset()) {
        ...
        const families = new Set(MODELS.map(item => tagFamily(item.tag)));
        const extra = [...have].filter(tag => !families.has(tagFamily(tag))).map(tag => ({ tag, label: tag }));
        return [...known, ...extra];
    }
